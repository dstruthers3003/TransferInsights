...
Who you are drawn against each gameweek, the eleven they would field, and the
projected margin. Their side is picked by the same optimiser as yours, so it is
their *best available* team rather than whatever they happen to have set.

Two things to know. League fixtures reference `league_entry` ids, which are not
the `entry_id` used for ownership - mapping between them is the one place this
would silently show the wrong squad, so it is mapped explicitly through
`league_entries`. And a league with an odd number of managers carries a dummy
AVERAGE side so everyone gets a fixture; it has no squad, so it is scored as
the mean projected eleven across the league.

Margins under about two points are reported as level rather than a result,
because that is inside the error on a projection of this kind.
